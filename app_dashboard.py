...
def display_dashboard():
    ...
    # --- Sidebar ---
    # ลบปุ่มออกจากระบบออกจาก Sidebar ทำให้ Sidebar ว่าง (หรือจะใส่เมนูอื่นในอนาคตก็ได้)
    with st.sidebar:
        st.header("เมนู")
        st.info("แดชบอร์ดนี้แสดงข้อมูลการเข้า-ออกงานส่วนบุคคล")

    # --- Main Content ---
    # --- ส่วนหัว (Header) ที่มีปุ่มออกจากระบบ ---
    # แบ่งพื้นที่ส่วนหัวเป็น 2 ส่วน: 80% สำหรับไตเติ้ล, 20% สำหรับปุ่ม
    title_col, logout_col = st.columns([0.8, 0.2]) # <--- แก้ไขส่วนนี้
    ...
    with logout_col:
        # ทำให้ปุ่มอยู่ชิดขวาและเยื้องลงมาเล็กน้อยเพื่อความสวยงาม
        st.write("") # เพิ่มบรรทัดว่าง
        st.button("🚪 ออกจากระบบ", on_click=logout, use_container_width=True) # <--- ปุ่มย้ายมาอยู่ตรงนี้
